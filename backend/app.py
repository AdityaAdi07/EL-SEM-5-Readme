...
# ========================
@app.get("/history")
def get_history(station_id: str = None, limit: int = 24):
    try:
        csv_path = "../frontend/data/live_data.csv"
        df = pd.read_csv(csv_path, parse_dates=["datetime"])
        ...
        # Sort and take last 'limit' records
        df = df.sort_values("datetime").tail(limit)
        
        history = []
        for _, row in df.iterrows():
            pm25 = float(row["PM2.5"]) if pd.notna(row.get("PM2.5")) else 0.0
            history.append({
                "time": str(row["datetime"].strftime("%H:%M")),
                "datetime": str(row["datetime"]),
                "PM2.5": pm25,
                "PM10": float(row["PM10"]) if pd.notna(row.get("PM10")) else 0.0,
                "NO2": float(row["NO2"]) if pd.notna(row.get("NO2")) else 0.0,
...
def analyze(data: dict):
    ...
    prompt = f"""
    You are an AI Expert & Environmental Scientist. Provide a highly structured, professional, and visually appealing analysis.

    ### 🌍 **PART 1: PREDICTION REASONING (The "Physics")**
    *Current Scenario Analysis:*
    {condition_desc}

    **Why "{event_status}" & "{reg_status}"?**
    *   **Lag Effects**: Analyze the influence of past 24h data.
    *   **Meteorology**: How humidity/season/time impacts this.
    *   **Physics**: Explain the dispersion/accumulation mechanism.

    ---

    ### 📊 **PART 2: MODEL SUPERIORITY (The "Data Science")**
    *RMSE Comparison:*  
    ✅ **XGBoost (15.94)** vs 🔸 Random Forest (16.22) vs 🔻 Decision Tree (18.02)

    **Why XGBoost Wins Here:**
    1.  **Gradient vs. Bagging**: Explain why boosting works better for these non-linear patterns.
    2.  **Bias & Variance**: How it handles the trade-off better.
    3.  **Feature Interactions**: Mention its superior handling of complex dependencies.

    *Review output for clarity, use bolding for key terms, and ensure a professional tone.*
    """
