...
export const getHistory = async (stationId, limit = 24) => {
    try {
        const response = await api.get(`/history?station_id=${stationId}&limit=${limit}`)
        return { success: true, data: response.data }
    } catch (error) {
        console.error('Fetch History Error:', error)
        return {
            success: false,
            error: error.response?.data?.detail || error.message || 'Failed to fetch history data'
        }
    }
}
